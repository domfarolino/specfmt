...
use clap::Parser;
use specfmt::{apply_diff, parse_diff_line_numbers, rewrap_lines, Line};
use std::fs::read_dir;
use std::fs::File;
...
fn main() {
    ...
    let rewrapped_lines = rewrap_lines(lines, num_lines_to_format, args.wrap);
...
mod test {
    ...
    fn simple_rewrap_tests(input: &str) {
        ...
        // Initiate unwrapping/rewrapping.
        let wrapped_lines = rewrap_lines(lines, length, 100);
